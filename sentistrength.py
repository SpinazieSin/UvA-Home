...
def prior_sentiment(contextual_features, input_file, jar_file, data_location):
	"""Enriches the contextual_features dict with prior sentiment scores.
	...
	"""
	command = ["java", "-jar", jar_file, "sentidata", data_location, "input", input_file]
	subp = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	time.sleep(1)
	file = open(input_file + sentistrength_postfix, 'r')
	file.readline()
	for line in file:
		tokens = [token.strip() for token in line.split('\t')]
		contextual_features[tokens[2]][1] = (int(tokens[0]) + int(tokens[1]))/4.0
	file.close()
